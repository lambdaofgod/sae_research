"""
Temporal SAE architectures.

Migrated from forks/temporal-saes/src/temporal_sequence_top_k.py.
Two variants:
- TemporalMatryoshkaBatchTopKSAE/Trainer: Matryoshka group-based reconstruction + temporal loss
- TemporalBatchTopKSAE/Trainer: simpler batch top-k (has known FIXME in encode, empty temporal_loss)
"""
...
class TemporalMatryoshkaBatchTopKSAE(Dictionary, nn.Module):
    ...
    def from_pretrained(
        ...
        autoencoder = cls(
            activation_dim, dict_size, k=k, group_sizes=group_sizes, temporal=temporal
        )
    ...
    @classmethod
    def from_config(
        cls, ae_path: str, trainer_cfg: dict, device: str
    ) -> "TemporalMatryoshkaBatchTopKSAE":
        return cls.from_pretrained(
            ae_path,
            k=trainer_cfg["k"],
            temporal=trainer_cfg["temporal"],
            device=device,
        )
...
class TemporalMatryoshkaBatchTopKTrainer(SAETrainer):
    ...
    def loss(self, x, step=None, logging=False):
        ...
        if self.temporal:
            ...
            l2_loss = (
                (x[:, 0] - x_reconstruct).pow(2).sum(dim=-1) * self.group_weights[0]
            ).mean()
            ...
            temp_loss = (
                t.abs(f_chunks[0] - f_temp_chunks[0]).sum(dim=-1)
                * x_temp_similarity
                * self.group_weights[0]
            ).mean()

            if self.contrastive:
                logits_per_curr_t = f_chunks[0] @ f_temp_chunks[0].T
                logits_per_cumsum_t = logits_per_curr_t.T

                labels = t.arange(
                    logits_per_curr_t.shape[0], device=self.device, dtype=t.long
                )
        ...
        for i in range(start, self.ae.active_groups):
            ...
            if self.temporal:
                l2_loss = (x[:, 0] - x_reconstruct).pow(2).sum(
                    dim=-1
...
class TemporalBatchTopKSAE(Dictionary, nn.Module):
    """Simpler temporal batch top-k SAE (non-Matryoshka).

    NOTE: This variant has a known FIXME in encode() where the temporal
    dimension is simply discarded (x[:, 0]), and temporal_loss() is empty.
    ...
    def encode(
        ...
        # NOTE: Temporal slicing (x[:, 0]) is handled by the trainer's loss(),
        # not here. This method always expects [batch, d_model] input.
    ...
    def from_config(
        ...
        return cls.from_pretrained(
            ae_path,
            k=trainer_cfg["k"],
            temporal=trainer_cfg.get("temporal", False),
            device=device,
        )


class TemporalBatchTopKTrainer(SAETrainer):
    """Simpler temporal batch top-k trainer (non-Matryoshka).

    NOTE: temporal_loss() is a no-op stub. The loss() method encodes both
    current and prior tokens but only uses the reconstruction loss on current.
    Copied as-is from the fork.
    """
