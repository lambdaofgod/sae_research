...
def create_activault_buffer(activault_config, sae_batch_size: int, device: str):
    """Create an ActivaultS3ActivationBuffer from an ActivaultConfig.

    Returns (buffer, metadata) where metadata is the S3 metadata dict
    containing shape, dtype, etc.
    """
    ...
    cache = S3RCache(
        s3_client=s3_client,
        s3_prefix=activault_config.s3_prefix,
        bucket_name=activault_config.s3_bucket,
        device=device,
        buffer_size=activault_config.s3_buffer_size,
        return_ids=True,
        shuffle=True,
