...
def test_sae_setup():
    ...
    teacher_model_reference = SentenceTransformer(
    ...
    for p1, p2 in zip(
        sae_wrapper.teacher.parameters(), teacher_model_reference.parameters()
    ):
        assert torch.equal(p1, p2), "Teacher model was modified during training!"

    # Test save/load
    sae_wrapper.save("experiments/embedder_sae/tmp")
    loaded_wrapper = SAEWrapper.load("experiments/embedder_sae/tmp")
    assert loaded_wrapper.encode(["Test"]).shape[1] == hidden_dim
