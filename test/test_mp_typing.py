...
assert median.shape == (activation_dim,), (
    f"Expected shape (activation_dim,), got {median.shape}"
)
...
assert z.shape == (n_tokens, dict_size), (
    f"Expected shape ({n_tokens}, {dict_size}), got {z.shape}"
)
print("✓ encode() works")
...
assert z.shape == (n_tokens, dict_size), (
    f"Expected z shape ({n_tokens}, {dict_size}), got {z.shape}"
)
assert indices.shape == (n_tokens, s), (
    f"Expected indices shape ({n_tokens}, {s}), got {indices.shape}"
)
assert initial_acts.shape == (n_tokens, dict_size), (
    f"Expected initial_acts shape ({n_tokens}, {dict_size}), got {initial_acts.shape}"
)
...
assert x_hat.shape == (n_tokens, activation_dim), (
    f"Expected shape ({n_tokens}, {activation_dim}), got {x_hat.shape}"
)
print("✓ decode() works")
...
assert set(nested_codes.keys()) == set(s_values), (
    f"Expected keys {s_values}, got {list(nested_codes.keys())}"
)
for s_val, code in nested_codes.items():
    assert code.shape == (n_tokens, dict_size), (
        f"Expected shape ({n_tokens}, {dict_size}) for s={s_val}, got {code.shape}"
    )
